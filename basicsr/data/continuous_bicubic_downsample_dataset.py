...
class ContinuousBicubicDownsampleDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        guide_img_path = guide_img_path.replace('t2', 't1')
        ...
        # crop_guide = guide_img[start_h_crop_gt:start_h_crop_gt + gt_size[0], start_w_crop_gt:start_w_crop_gt + gt_size[1],
        #              :]
        ...
        # img_gt, img_lq, guide_img = augment([crop_gt, crop_lr, crop_guide], self.opt['use_hflip'], self.opt['use_rot'])

        # # color space transform
        # if 'color' in self.opt and self.opt['color'] == 'y':
        #     img_gt = bgr2ycbcr(crop_gt, y_only=True)[..., None]
        #     img_lq = bgr2ycbcr(crop_lr, y_only=True)[..., None]
        ...
        if self.sample_size > 0:
            ...
            # pad_h = 8 - img_lq.shape[1] % 8
            # pad_w = 8 - img_lq.shape[2] % 8
            pad_h = 0
            pad_w = 0
        ...
        # print(f'img_gt shape: {img_gt.shape}, img_lq shape: {img_lq.shape}, img_guide shape: {img_guide.shape}')
        ...
        if sample_coords is not None:
            ...
            return_d = {'gt': img_gt, 'lq': img_lq, 'guide':img_guide, 'scale': scale,
                        'gt_size': gt_size, 'pad_h': pad_h, 'pad_w': pad_w, 'scale_modify': scale_modify, 'gt_path': img_path}
        return return_d
...
def fft_based_downsample(img_3d, scale_h, scale_w):
    ...
    # Zero-padding 回原始尺寸
    kspace_padded = np.zeros_like(dft_shifted)
    kspace_padded[start_row:end_row, start_col:end_col] = kspace_cropped

    f_ishift = np.fft.ifftshift(kspace_padded)
